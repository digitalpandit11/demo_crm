...
class Timesheet extends CI_Controller {
    ...
    {
        $data['employee_list'] = $this->timesheet_model->get_employee_list();
        $this->load->view('support/timesheet/vw_timesheet_main_index',$data);
    }


    public function employeewise_timesheet_index()
    {
        $employee_id = $this->uri->segment(2);
        $data['employee_id'] = $employee_id;
        $data['employeewise_activity_list'] = $this->timesheet_model->get_employeewise_timesheet($employee_id);
        $employee_details = $this->timesheet_model->get_employee_details_by_id($employee_id);
        $data['employee_details'] = $employee_details;
        // echo'<pre>';
        // print_r($data['employeewise_activity_list']);
        // die();
        $this->load->view('support/timesheet/vw_timesheet_employeewise_index',$data);
    }
    ...
    public function add_activity()
    {
        ...
        $this->load->view('support/timesheet/vw_timesheet_calendar',$data);
    }
   
    public function get_employee_calender()
    {
        $employee_id = $this->input->post('employee_id');
        $this->db->select('*');
        $this->db->from('activity_timesheet_relation');
        //$this->db->join('','','');
        //$where = '(entity_id = '.1.')';
    ...
    public function add_employee_activity()
    {
        $employee_id = $this->input->post('employee_id');
        $activity_detail = $this->input->post('event_name');
        $start_datetime = $this->input->post('event_start_date');
        $end_datetime = $this->input->post('event_end_date');
        $activity_type = 1;
        $status = 1;

        $activity_insert_array = array(
        ...
        $this->db->insert('activity_timesheet_relation',$activity_insert_array);
