...
<head>
	...
	<title>Select Indiamart API</title>
				...
				<div class="container-fluid">
					<div class="card">
						<div class="card-header">
							<h1 class="card-title">Generate India Mart Lead</h1>
							<div class="col-sm-6">
								<br><br>
								<ol class="breadcrumb">
					...
					<div class="row">
						<div class="col-md-12">
							<div class="card card-primary">
								<div class="card-header">
									<h3 class="card-title">India Mart Lead Lists </h3>
								</div>
												...
												<table id="example1" class="table table-bordered table-striped">
													...
													<tbody>
														...
														foreach ($enquiry_details as $row) :
															...
															$Lead_date_format = $row->lead_date;

															$Lead_date = date('Y-m-d', strtotime($Lead_date_format));
														?>
															<tr>
																<td style="display: none;" class="relation_id" id="relation_id"><?php echo $entity_id; ?></td>

																<!-- <td><input type="checkbox" class="checkboxes" id="india_mart_id" name="india_mart_id" value="<?php echo $entity_id; ?>"></td> -->
																<td>
																	<a onclick="return confirm('Are You Sure To Create Lead?')" href="<?php echo site_url('create_indiamart_lead/' . $entity_id); ?>" class="btn btn-sm btn-info"><i class="fas fa-check"></i></a>
																</td>
																...
																<td><?php echo $Lead_date_format; ?></td>
																<td><?php echo $row->lead_type; ?></td>
																<td>
																	<textarea class="form-control" id="details_of_lead" name="details_of_lead" disabled style="width: 500px;" rows="5"><?php echo $row->api_details; ?></textarea>
																</td>
																...
																<td>
																	<textarea class="form-control" id="lead_detail" name="lead_detail" disabled style="width: 300px;" rows="3"><?php echo $row->lead_detail; ?></textarea>
																</td>
																...
																<td class="indiamart_status" id="indiamart_status">
																	...
																	<a class="btn btn-sm btn-danger disqualify_btn"  data-toggle="modal" data-target="#modal-disqualify" data-indiamart="<?php echo $entity_id; ?>"><i class="fas fa-thumbs-down"></i> </a>
																</td>
															</tr>
														<?php endforeach; ?>
													</tbody>
...
<div class="modal fade" id="modal-disqualify">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form role="form" name="lead_disqualify" id="lead_disqualify" method="post">
						...
						<input type="text" id="india_mart_id" class="india_mart_id text-secondary" style="border: none; font-size:10px;" required readonly>
						</div>
						<div class="row">
	...
	<script>
		...
		$(document).on('click','.disqualify_btn', function(){
			...
			$('.india_mart_id').val('');
			var india_mart_id = $(this).data('indiamart');
			$('.india_mart_id').val(india_mart_id)
		});

		$('#confirm_disqualify').click(function(){
			var india_mart_id = $('.india_mart_id').val();
			var reason_to_disqualify = $('#reason_to_disqualify').val();
			
			if(india_mart_id != "" && reason_to_disqualify != "")
			{
	...
	<script>
		$(function() {
			$("#select_lead").on('click', function(event) {
				...
				var india_mart_id = [];
				$.each($("input[name='india_mart_id']:checked"), function() {
					india_mart_id.push($(this).val());
				});
